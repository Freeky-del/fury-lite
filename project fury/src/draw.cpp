...
void draw::draw_skeleton(ImDrawList* draw_list, const esp_entity& entity)
{
	for (const auto& connection : boneConnections) {
		...
		if (entity.boneValid[b1_idx] && entity.boneValid[b2_idx]) {
			...
			if(globals::esp::joints) draw_list->AddCircleFilled(p2, globals::esp::skeleton_thickness + 1.0f, getARGB(colors::esp_color));
		}
	}
}
...
{
	ImVec2 origin;
	Vec3 head = entity.bones[bones::head];

	if (wts(head, globals::ViewMatrix, menu::width, menu::height, origin.x, origin.y)) {
